...
- **數學（三下·康軒·期末）**：清單＝`data/tcool_grade3_math_kanghsuan.json`（62 筆，掃描器 `scripts/sweep_tcool_math.ps1`）。現行課綱期末共 15 份，**有答案卷 6 份全數已下載**至 `pdfs_數學/`：
  - 第一批（已上線）：桃子腳 112下、安和 113下 — 格式A，extract_math pipeline。
  - 擴充批（2026-06-11 下載）：桃子腳 110下、安和 111下、彰化中正 111下、彰化田中 111下 — 版面各異（桃110 假粗體四重字／中正答案緊貼括號且選擇題答案不在文字層／安和111 與田中111 為題目卷＋獨立答案表），extract_math 不適用，走**人工策展＋官方答案＋PNG 逐題核對**入庫，剔除清單見 `skipped_questions.md`。
  - 其餘 9 份無答案卷（建德113、民權113、內湖113/112、社寮112/110、草港112、舊館111 期末3、吉林111）＝「AI 補答案＋複審」批次，未做。
  - 注意：110/111 學年度數學卷含「長度（公分/毫米）」內容，不在現行單元 5–9 taxonomy → classify 標 none 自動排除，屬預期行為。
