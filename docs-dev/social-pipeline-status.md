...
**後續**：✅ 新增 226 題作答後說明已完成（見下節）。剩看圖題策展（甲乙標章/此標章等），可再收更多無答案卷。

## ✅ 無答案卷批 226 題作答後說明已完成（2026-06-15，路 B 雙盲）

社會 226→452 的 226 新題全補作答後說明，社會 452 題全覆蓋、`explanations.json` 1031→**1257**。branch `feat/social-noans-explanations`。

- **流程**＝路 B 雙盲：12 sonnet 寫手 round-robin 分批（各 18–19 題，附主迴圈已核定典故事實 brief）→ 8 獨立盲審 agent（連續切 8 批、不給 brief、獨立判事實/答案一致/小三用語）→ 主迴圈裁決 **213 pass／13 fixed**。
- **關鍵差異**＝這批答案是 **AI 補的（無官方答案卷，原全 needs_review）**。brief 明示寫手/審核遇「強烈覺得答案事實有誤」改加 flag、不硬掰背書（沿用 pilot flag 機制）。寫手 3 flag、盲審 14 不通過，去重 14 題進主迴圈裁決。
- **13 改寫**：射日傳說聚焦布農族非泰雅（避免籠統否定）、鹽埕不拆「埕」字（埕本義空地）、鹹菜刪「非閩南」絕對否定、新埔記錄開墾非生產物、造橋刪 PDF 滲入錯誤河名「大甲溪」（造橋在中港溪流域）、元長鄉刪查無依據的「名叫元長的人」、高雄/民雄古地名改寫（原把答案寫反＋民雄誤寫「蔴里蘭」→正確打狗/打貓都帶「打」字、名稱類似）、竹山客運站「客運是否復駛」邏輯反、馬公天后宮「無從確定」改點明②錯在「面積最大」、喜憨兒公益消費誤掛「綠色消費」改說助人消費、坪林刪「林溪」（改地形平坦＋林木茂盛）、山腳國小日式建築刪課本未明示的「教學場地」、舊館 tf_13 鹽埕例子框成「居民從事的活動」。
- **審核 2 處誤判主迴圈駁回**：三井 LaLaport 台中館實際在**臺中市東區**（進德路，近台中車站；審核混淆成北屯/梧棲的「三井 OUTLET」），題目與原說明正確 → 維持原稿。
- **無答案需改**：14 flag 全屬說明層問題，無一指出題庫 AI 答案本身錯誤（answer 不動）。
- **程式/前端不改**：`build_explanations.py` 社會漸進覆蓋（452∩entry=452 完整）、前端說明卡通用。批次檔＝`data/exp_results/batch_social_11..14.json`（格式 `{id,text,verdict,reason?}`）；抽查＝`docs-dev/review_社會說明_抽查.md`（452 題、改寫 35＝前批 22＋本批 13）。tests **116 passed**、Playwright social join 452/452＋改寫題顯示正確。寫手/審核中間暫存檔已清。
