...
### 最終抽查（3 獨立 agent 隨機 45 題）＋順手修 3 題題庫破損
- 完成說明後家長要求 **3 agent 隨機抽查 45 題（固定種子、各 15、涵蓋 9 卷 unit 均衡）**做最後確認：**無一答案錯誤**，僅低優先用詞建議。據此微調 2 說明（高雄「荷蘭人沒有參與其中」→「更名過程不涉及荷蘭人」、打狗/打貓「聽起來很像」→「看起來很相似」）。
- 抽查順帶揪出 **3 題題庫題幹/選項萃取破損**（無答案卷 AI 補答案批就已存在，與本次說明無關，但會在小孩螢幕破版）→ 就地修 source `classified_questions_社會.json` + rebuild：
  - 文心112 **mc_13 造橋**、**mc_14 布袋戲**：PDF 雙欄把「張達京／葫蘆墩圳」閱讀題那欄滲入題幹與 4 選項 → 重建乾淨題幹/選項（答案 ①造橋／③雲林布袋戲館不變）。
  - 太平112 **mc_1 消費專線**：選項解析壞成 3 個 `['1950','5','113']` → 修回 `1950/1590/113/165`（答案 ①1950 不變）。
- 複驗：全 244 社會 mc 皆 4 選項、無殘留欄位滲漏、社會 452/452 join、tests 116 passed、Playwright 3 題渲染乾淨。
