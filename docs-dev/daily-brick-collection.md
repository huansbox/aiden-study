...
本次工作台拼上最後一組，待吸附動畫結束、伺服器確認完成且圖片載妥後，自動播放一次 4.2 秒的 E500 試車。沿用正式 LEGO style 圖片，三組展示軌道保持固定，其餘 39 組組成整台火車沿軌道來回，再停回展示位置；後半段灑下少量彩色小積木。正式頁面仍不使用 WebGL。
...
開發預覽服務的 `/celebration.html` 提供 41／42 組的示範進度，可點擊或拖曳最後一組，試看動畫、略過、重播與上架。示範進度只存在記憶體，不寫入家庭資料。
...
- 已整合主線自然題庫第一批（PR #118），保留既有每日目標與收藏接線。Node 全套 670 項、Python 269 項通過，Python 1 項略過。
- 更新後的 [20 條 E2E](e500-e2e-result.json) 全數通過：實際完成 42 組後驗證三組軌道固定、39 組火車移動、自動結束、手動重播、略過，再上架收藏。另以示範頁驗證直向 Pad 觸控拖曳、上架及 reduced-motion 直接完成；未進行實體 iPad 測試。
- 工作台 38 項測試涵蓋本次最終放置／伺服器確認、4.2 秒生命週期、圖片錯誤重試、關閉清除計時及舊收藏不自動播放。獨立唯讀 review 發現的跨裝置舊快取誤播已修正，補上 41 組與 42 組待同步快取的回歸測試，複查無未解決 P1／P2。
