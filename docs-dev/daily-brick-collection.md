...
家長後台的「孩子設定與練習 → 每日目標與拼裝包」提供「火車試拼」，開啟獨立的 [試拼頁](../docs/parent/train-preview.html)。三款車都能從第一包開始，逐包試到完工；也能直接保留最後一組，試拖曳、六秒慶祝、音效、略過、重播及示範展示架。從家長入口開啟後，返回時保留原本管理的孩子。
...
第一系列為「臺灣火車系列」：E500 需 14 包、42 組；EMU3000 與 R200 各需 12 包、36 組。每包三組可操作部件，每日兩包約六至七個達標日完成一件；漏日不扣除成果。每組由多顆積木組成，並非要求孩子每次只放一顆最小積木。
...
新版可選模型包含 E500、EMU3000 與 R200；完成一台後可選下一台，收齊三台後額外拼裝包繼續保留。舊版小汽車、蒸汽火車與飛機以原 ID、原素材保留既有半成品及完成收藏，不把舊火車進度直接改成 E500；舊客戶端的已支援模型操作仍可接續。
...
本次工作台拼上最後一組，待吸附動畫結束、伺服器確認完成且圖片載妥後，自動播放一次 6 秒試車，三款列車皆支援。沿用正式 LEGO style 圖片，E500 的 39 組車體、或 EMU3000／R200 的 33 組車體從右後方進場，經過中央後向左前方駛出，全程單向、不倒退；原本三組展示軌道沿投影方向延伸為九張固定素材，列車駛過時灑下少量彩色小積木。正式頁面仍不使用 WebGL。
...
完整流程先真正練習領兩包，再透過只存在測試服務的 `/test/collection-fixture` 補十二個過去日期的包，以免等待多日。E500 完成後累計補至三十七個過去日期、合計 39 包，再依序完成 EMU3000 與 R200，留下 1 包未分配。114 組均逐件經畫面操作和正式收藏 API 保存。另用獨立 browser context 驗證另一台裝置、離線重開，以及維運同型的 activity generation 重置。
