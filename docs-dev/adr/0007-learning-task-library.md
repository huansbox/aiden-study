...
決定（2026-08-14）：新增 `learning-tasks/` 作為家庭學習任務庫。工作一旦判定為 task，就立即建立獨立資料夾並以「進行中」狀態加入索引，不等完成後才搬入；`learning-tasks/README.md` 是索引與操作規則的唯一真相源。Codex 的 `AGENTS.md` 與 Claude Code 的 `CLAUDE.md` 都只負責把新 session 導向該索引。
