...
使用者完成 14 段錄音，並在比較 AirPods／MacBook Pro 後選擇 MBP。此包全部採用 Safari 回報的「MacBook Pro的麥克風」，不混用 TTS 或 AirPods。先前三段 MBP 原音與後續十一段都保留；正式資產路徑為 `docs/zhuyin/assets/audio/`。本包已由統籌選入正式音檔交付；發布證據見 [MVP 收尾紀錄](../zhuyin-mvp-ipad-checklist.md)。
...
重建直接使用本包保存的原音與指定裁切點，無需再次錄音。原音 hash 或內容 key 不符會拒絕重建；先在暫存處完成全批轉碼，再寫入正式資產路徑。同一 MBP 環境連續重建後 14 個成品 hash 完全相同；不同 ffmpeg 版本不保證位元一致。Windows 也已驗證能從保存的原音重建 14 段，但本次出貨維持使用者試聽接受的 MBP 成品，沒有重新編碼替換。
...
先前 [TTS 候選包](https://github.com/huansbox/aiden-study/tree/codex/zhuyin-audio-candidates/docs-dev/zhuyin-audio-candidates) 保持獨立，僅作歷史與來源對照；本批出貨資產不使用任何 TTS 片段。TTS 候選與本機錄音試驗工具留在原分支；正式分支只收親錄來源、已接受成品與必要重建／審計工具。`acceptance.json` 的 `deployed: false` 是使用者試聽當時的歷史狀態；目前發布狀態見收尾紀錄。

## Windows 整合檢查（2026-09-17）

- 14 段原音與成品均符合來源、recipe、build 與 audit 的 hash；全部可解碼，缺檔與孤兒為 0。
- 修正 recipe 在 Windows 自動換行造成的 hash 差異，以 `.gitattributes` 固定該檔 LF；Python 讀寫明定 UTF-8。
- Node 341 passed；pytest 183 passed、1 skipped。
- 隔離 test-token／記憶體 KV 的孩子入口可進第一張注音介紹卡；家長音效自檢顯示 14 段齊備並啟動連播。未在正式站製造練習進度。
