...
test("importedFeedbackText：anchor-failed（定錨標記寫失敗）明說可能不會上雲、不承諾補傳", () => {
  const af = importedFeedbackText("弟弟", false, "anchor-failed");
  assert.match(af, /已寫入這台裝置/);
  assert.match(af, /空間不足/);
  assert.match(af, /可能不會自動上雲/);
  assert.doesNotMatch(af, /會自動補傳/, "定錨沒寫進去，補傳承諾不成立");
});
