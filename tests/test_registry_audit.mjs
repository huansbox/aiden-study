...
test("sync 設定一致：sync=true 必為站內 app、頁面引用 sync client、雲端 key 段＝registry id", () => {
  ...
  for (const a of (reg.apps || []).filter((x) => x.sync === true)) {
    ...
    // 0 或多個都當髒（響亮失敗勝過默過）。
    // TODO(#40-B 過渡)：四 app 重接完成前，未重接的 app 退回舊 *_APP_ID 慣例——收尾 commit 移除退路
    ...
    if (ids.length !== 1) { errs.push(`app ${a.id} 頁面應恰有一個 <wiring-config> 或 *_APP_ID 常數，找到 ${ids.length} 個 APP_ID`); continue; }
