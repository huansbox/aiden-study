...
2. 每概念重新設計 Try it／Say it 各三題。Try依序為 `stage: build/type: order`、`stage: change/type: order`、`stage: fix/type: repair`：組完整句、依新條件實際改句、必要時單字修正。Change不只重複換名詞／數字；Fix是補強，只有一個明確錯字。孩子端跨概念做Build再Change，前兩題皆獨立成功省略Fix。通常1個、最多2個混淆字；Say仍列自然完整句與合理替代答案。換情境或可見證據、保留目標能力；不能只換ID或照抄舊題。新包直接寫 `concept.try`，不需要為初版建立tryRevision。題目ID在包內唯一。
