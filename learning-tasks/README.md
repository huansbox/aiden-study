...
4. 任務完成後，把可重建成果歸檔成獨立資料夾並更新本索引。
5. 同一原則、模板或素材在第二個不同任務實際重用後，再整理到 `shared/`；第一次出現時先留在原任務，避免過早抽象。

## 任務索引

| 任務 | 日期 | 對象 | 情境與玩法 | 可重用經驗／素材 |
| --- | --- | --- | --- | --- |
| [新竹動物園小小探險](hsinchu-zoo-adventure/) | 2026-08-14 | 5.5 歲弟弟；一位成人陪同 | 火車地理、園區選路、動物特徵觀察、昆蟲分類；卡片可任意中止 | [幼兒現場觀察任務卡經驗](hsinchu-zoo-adventure/design-lessons.md)、大 Poker／A4 二分標籤卡來源、圖資與 PDF |
...
- 已完成且沒有後續工作的任務不另開 issue；commit、task README 與本索引就是歷史入口。
