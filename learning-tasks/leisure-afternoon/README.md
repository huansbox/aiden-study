...
- 狀態：2026-09-15 已合併 master 並上線；使用者決定略過額外驗收，孩子可直接使用。
...
1024×650 的五步按鈕與完成圖操作列可見；完成圖另驗 1024×768、1180×720、768×1024、390×844。文字盒無互相重疊、SVG 連線取樣未進入文字周圍 3px 範圍。iPad 真機與列印未另實測；2026-09-15 使用者決定略過，不列待辦。
...
2026-09-15 使用者明確決定略過額外驗收，孩子直接使用正式頁面。本任務沒有待驗收或待續實作；[驗收紀錄](source/coverage.md) 保留已測與略過的區別，不將未測項目標為通過。若老師再次要求補重點，先回到段落對照判斷漏掉的內容，再調整該篇，避免直接增加每個分支的字數。
