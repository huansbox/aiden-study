...
http.createServer(async(req,res)=>{
 try{
  ...
  else if(pathname.startsWith('/shared/')){base=path.join(repository,'docs/shared');relative=pathname.slice('/shared'.length);}
  ...
  const type={'.html':'text/html; charset=utf-8','.js':'text/javascript','.css':'text/css; charset=utf-8','.json':'application/json','.png':'image/png'}[path.extname(file)];
