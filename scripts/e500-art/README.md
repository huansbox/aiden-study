...
EMU3000 與 R200 沿用同一管線；腳本保留原 E500 名稱與預設參數，重建既有素材的指令仍相容。
...
完整模型與慶祝頁均支援 `?model=e500`、`?model=emu3000`、`?model=r200`，頁首可切換車型。EMU3000／R200 的慶祝示範為 35／36 組。可用 `node scripts/e500-art/verify-celebration.mjs <CDP endpoint> http://127.0.0.1:8877 emu3000`（或 `r200`）驗證新車；結果按車型命名。
...
模型測試現涵蓋三台：共 198 張 PNG（包含變體），每包六種順序共 684 次放置皆須有可見像素變化。新車各 216 次，E500 252 次。`capture-model.mjs` 仍是 E500 專用幾何操作檢查；新車使用通用 `index.html?model=…` 與輸出比對頁檢視，並在 `verify-collection-e2e.mjs` 實際完成組裝。
