# 掃描 tcool.cc 全台「三下·自然·康軒」清單（空 city + 空 period，分頁到底）
# 輸出 data/tcool_sweep_all.json，事後再過濾期末+110下。
$ErrorActionPreference = "Stop"
$out = @()
$prevSig = ""
for ($p = 1; $p -le 60; $p++) {
    $r = Invoke-WebRequest -Uri "https://www.tcool.cc/" -Method POST -Body @{
        grade="3"; subject="自然"; semester="2"; period=""; publisher="康軒"; city=""; p="$p"
    } -UseBasicParsing -TimeoutSec 40
    $html = $r.Content
    # 切出每個 result 區塊
    $blocks = [regex]::Matches($html, '(?s)<div class="result">(.*?)</div>\s*</div>\s*</div>')
    if ($blocks.Count -eq 0) {
        # 後備：用較寬鬆切法（以下一個 result 或結尾為界）
...
$dedup | ConvertTo-Json -Depth 5 | Out-File -FilePath "data/tcool_sweep_all.json" -Encoding utf8
Write-Host "TOTAL unique: $($dedup.Count) -> data/tcool_sweep_all.json"
# 順手印出期末+110下+ 的清單摘要
...
$dedup | Where-Object { $_.year_period -match "期末" } | ForEach-Object {
    ...
    if ($yr -ge 110) { "{0,-6} {1,-10} {2,-14} ans={3}" -f $_.city, $_.school, $_.year_period, [bool]$_.a }
}
