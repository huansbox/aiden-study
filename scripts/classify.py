...
SOCIAL_SYSTEM_PROMPT = """你是國小三年級下學期社會「期末」考題分類專家。期末範圍為康軒版三下單元四～單元六
（單元名稱與多來源交叉查證，與桃子腳國小卷面內容吻合），請將每道題目分類到對應的單元與子主題
（以桃子腳國小範圍為基準）：

第4單元：消費與選擇（內部 unit 編號 10）
- 聰明消費：選購與保存食品的考量、檢查商品標示/標章/有效期限、消費流程、安全帽選購、電子發票、消費爭議與權益（退換貨、1950消費者服務專線、1999/消費者保護官）
- 綠色消費：環保消費、減少污染與過度包裝、選購在地當季食材、珍惜食物與資源、節能減碳

第5單元：家鄉的地名（內部 unit 編號 11）
- 地名的由來：地名與自然環境（地形/植物/氣候）、地名與居民活動（開墾/生產/建築）、與原住民族社名有關的地名
- 探索家鄉的地名：探索家鄉地名的步驟與方法（挑選地名→查資料→訪問長輩→整理→發表）、探索地名的好處

第6單元：家鄉的故事（內部 unit 編號 12）
- 家鄉的人物與發展：先民對家鄉的貢獻（興築水圳如施世榜八堡圳、文化技藝如洪鴦藺草等）、家鄉的開發與發展
- 傳說與文化保存：家鄉的傳說故事（半屏山、射日傳說、鶯歌石等）、古蹟與閒置空間活化再利用、人文風貌與文化特色的保存推廣

衝突規則：若題目同時符合兩個單元，優先歸屬「題目直接詢問的核心概念」所屬單元。
不屬於以上任何子主題的題目（超出期末範圍），unit 填 "none"。

對於每道題目，請回傳：
- unit: "10"、"11"、"12" 或 "none"
- subtopic: 上述六個子主題之一，若 unit 為 "none" 則填 "none"
- confidence: 0-100 的整數，表示分類信心
- classify_reason: 一句話說明分類理由
- answer: 正確答案（是非題填 "true" 或 "false"，選擇題填 "1"/"2"/"3"/"4"）。若題目已有答案則直接回傳該答案，若無答案請根據國小三年級社會科知識判斷正確答案。
...
SEMESTERS = {
    ...
    "social": {
        "input": os.path.join(DATA_DIR, "raw_questions_社會.json"),
        "output": os.path.join(DATA_DIR, "classified_questions_社會.json"),
        "valid_units": {"10", "11", "12", "none"},
        "valid_subtopics": {
            "聰明消費", "綠色消費",
            "地名的由來", "探索家鄉的地名",
            "家鄉的人物與發展", "傳說與文化保存",
        ...
        "unit_labels": {"10": "消費與選擇", "11": "家鄉的地名", "12": "家鄉的故事"},
        "model": "claude-sonnet-4-6",
        "batch_size": 15,
    },
}
