...
單元名稱暫依均一教育平台「類康軒版」對照（正式名稱由 issues/010 查證課本目錄後定案）：

單元5：小數 — 一位小數的認識、十分位、小數大小比較、小數與分數的關係、小數加減
單元6：圓 — 圓心/圓周/半徑/直徑的關係、圓規的使用、圓的大小比較
單元7：乘法與除法 — 乘除互逆（驗算）、整數乘除計算、兩步驟乘除應用問題
單元8：時間 — 時刻與時間量、24時制/12時制互換、時間的加減計算、合理時間單位
單元9：統計表 — 報讀功課表/票價表/時刻表等表格並回答問題

衝突規則：優先歸屬「題目直接詢問的核心概念」所屬單元。
不屬於以上任何單元的題目，unit 填 "none"。

對於每道題目，請回傳：
- unit: "5"～"9" 或 "none"
- subtopic: 一律填 "none"（本階段不分子主題）
- confidence: 0-100 的整數，表示分類信心
- classify_reason: 一句話說明分類理由
...
SEMESTERS = {
    ...
    "math": {
        ...
        "valid_subtopics": {"none"},    # 009 最小分類：subtopic 一律 none，正式 taxonomy 留給 010
