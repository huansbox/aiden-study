...
export class ChildCollection extends DurableObject {
  constructor(ctx, env) {
    ...
    this.sql.exec("CREATE TABLE IF NOT EXISTS practice (id TEXT PRIMARY KEY, round_id TEXT, entry_id TEXT NOT NULL)");
  }
  ...
  execute(command) {
    try { return this.ctx.storage.transactionSync(() => {
      ...
      if (command.type === "round") nextCommand.hasPractice = Boolean(this.sql.exec("SELECT id FROM practice WHERE round_id=? AND entry_id=? LIMIT 1", command.event?.roundId || "", command.event?.entryId || "").toArray().length);
      ...
      if (command.type === "record") this.sql.exec("INSERT INTO practice(id,round_id,entry_id) VALUES(?,?,?)", id, command.event.roundId || null, command.event.entryId);
