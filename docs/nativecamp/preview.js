...
(function (global) {
  ...
  function mount({ root, lesson, child = "aiden", auth = global.KidsAuth, makeAudio = () => new Audio(),
    ...
    function render() {
      ...
      const answers = mode === "try" ? `<div class="answer-area">${V.answerControls(q, selection, words)}</div>` : '<p class="parent-note answer-area">Say an answer, then reveal the example.</p>';
