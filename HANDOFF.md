...
- Branch: master
- Implementation: #56／#57／#58 已完成並關閉；W4 家長回報五項通過，完整驗收未結案
- Updated: 2026-09-14
...
2026-09-14 家長已回報：設定家庭金鑰後自動載入六題、重開接續並保留已送出進度、未送出輸入清空、哥哥／弟弟進度隔離、輸入與確認／離開按鈕操作正常。環境已知為 **iPad、橫向**；由 iPhone 透過 AirDrop 傳網址至 iPad 後直接開啟，尚未建立主畫面圖示；瀏覽器名稱、型號與 iPadOS 版本未知。這是家長操作證據，未讀取真 token、題文或孩子雲端進度，也未擷取真機 HTTP trace。

三下歷史進度／錯題由家長確認不再追回，該實際歷史資料對帳標為不適用；2026-07-17 已有放棄舊備份找回／對帳的決策。這不授權清除現有資料，也不取消 synthetic data 的年級隔離回歸或 #34 現有資料備份。

本次結果、證據界線與剩餘清單集中在 [`docs-dev/grade4-u1-ipad-acceptance.md`](docs-dev/grade4-u1-ipad-acceptance.md)。#55 仍 open，不因五項基本操作已確認就省略完整 W4。

## Next step

1. 先依驗收紀錄補記瀏覽器名稱與已測離開路徑；五項已回報結果保留；切到其他 App 再回 Study 的進度與操作也已另行確認，僅補尚未明確記錄的其他路徑。
