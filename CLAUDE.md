...
- `errorBank`: 錯題庫（去重）。**全部練習模式：累計答錯 ≥ 2 次才加入**（2026-06-17 起；一次粗心、同批內馬上自己改對的不算，`State.wrongCount`=stats.practiced−correct，recordAnswer 先累加故已含本次）；答對不移除。**錯題練習模式：答對才移除**。載入時會把不符新規則（有 stats 且 wrongCount < 2）的舊條目清掉以對齊。
