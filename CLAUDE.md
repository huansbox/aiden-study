...
- **▶ 注音學習 app（弟弟版 v1）已完成規劃、待實作（2026-07-02）**：對象＝二兒子 5.5 歲不識字。PRD＝[`issues/prd-zhuyin-app.md`](issues/prd-zhuyin-app.md)（GitHub issue 掛 `ready-for-agent`；規劃 session 與家長 8 題逐題對齊＋3 reviewer agent 審查修正已併入 PRD）。落點＝`docs/zhuyin/` 獨立單檔（獨立 key `aiden_zhuyin_v1`）、**GH Pages 不搬家**＋獨立 CF Worker 進度同步（整站遷 CF Pages 解耦為後續另案）。**開工前置＝PRD 末「⚠️ 待家長拍板」節：①聲音策略待 iPad spike 定案（spike 頁已搬部署＝`docs/zhuyin-tts-spike.html`→ https://huansbox.github.io/aiden-study/zhuyin-tts-spike.html ；自動播驗證已補 2026-07-10：第 6 區「開始」一次手勢後 TTS＋音檔兩條各連播 6 段、間隔遞增至 30 秒，本地 Chromium 兩條 6/6 過，**待家長 iPad 真機跑**）②家長開 CF 帳號（不開也可先上線、同步後補）③child 識別字串問一句**。分支 `claude/zhuyin-learning-app-ogk7u3` 已 merge master（2026-07-10）。
