...
- **▶ 待辦（後續工作）**：①期中自然（unit 1/2）作答後說明（考後再議）②隱藏題救回 3 題（考後再議）③社會後續：**路 A 擴充批＋路 B 擴充說明 166＋無答案卷 AI 補答案批 226＋無答案卷批 226 題作答後說明皆已完成（社會 452 題說明全覆蓋，explanations 1257；見下方無答案卷說明條目）**；剩 社會看圖題策展（甲乙標章/此標章等，比照數學看表題截圖）、可再收更多無答案卷。④**分批練習 PRD #2**：sub-issue #3–#6 全部完成並關閉（2026-06-15，見下方分批條目）；父票 PRD #2 暫留，唯餘 1 項 cosmetic 債（resume 半批進度條歸零）待家長定奪是否修。⑤**題幹列舉條列換行擴及選擇/是非題（設計已對齊 2026-06-15，定案做法 B，待實作）**：設計稿＝[`docs-dev/enum-break-design.md`](docs-dev/enum-break-design.md)。**定案＝純前端 regex 換行、零新檔零 pipeline**：render 時對切選項後的題幹偵測 CJK 序數列舉標記（括號式（甲）/(甲)、裸式 甲.甲：甲、），≥2 標記且「標記後有實質內容」才在標記前插 `<br>`。涵蓋 MC/TF/fill 三題型，抽純函式 `applyEnumBreaks` 配 `tests/test_breaks_pure.mjs`。**避坑記憶**：曾對齊「鬆撈→2 輪 agent 審→breaks.json 獨立 pass」重型設計，2 個 reviewer agent 一致否決（真列舉僅約 11–12 題、假陽性現庫不存在、獨立 pass 無 CI 會變死流程）→ 改 B。不收數字(N)/拉丁/羅馬(0筆)；萃取破損題與百葉箱選項滲漏不處理。見下方「填充題空格 chip＋條列換行」節。⑥**iPad 主畫面 App（standalone）「從備忘錄還原」走剪貼簿（家長 2026-06-15 定案暫緩、收工）**：見下方「PRD #7 備份到備忘錄」條目的限制說明。
