...
目前學習內容主軸是把已核歷屆題小批加入 iPad 題庫。[#55 四上數學 U1：歷屆題庫接入 iPad 練習](https://github.com/huansbox/aiden-study/issues/55) 已完成共用程式、首批六題 private pack 與原 manual import 的 desktop 整合驗證；最新核准的 [#58 家庭權限自動讀取](https://github.com/huansbox/aiden-study/issues/58) 已完成 code、獨立 review、Worker 與正式 KV pack 發布，讓 Study 可從原入口自動載入六題，不要求家長傳檔。舊三下題目與進度保留，production family-token GET 與真 iPad／家長驗收仍由 #55 W4 完成。執行狀態見 [`wiki/Plan.md`](wiki/Plan.md)，詳細整合方案見 [`docs-dev/grade4-u1-study-integration-plan.md`](docs-dev/grade4-u1-study-integration-plan.md)。
