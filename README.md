...
首頁的「閱讀小工坊」可開啟 `mind-map.html`。以司馬光與〈客中初夏〉的閱讀文章為材料，孩子用五步選詞組出自己的圖，最後切換成不超過 60 字的抄寫版。可隨時回頭改選；進度保存在目前瀏覽器。此頁與既有題庫紀錄分開。

心智圖採中央主題向四角分枝的畫法。整個閱讀流程使用字嗨注音黑體 Bold，中文字右側附注音；字型隨網頁載入，不需安裝。字型來源與授權見 [字型說明](docs/assets/fonts/README.md)。

本機啟動網站後開啟 `/mind-map.html`；iPad 需與電腦在相同區域網路，使用電腦的區域網路位址和伺服器連接埠。驗收紀錄見 [021](issues/021-reading-mind-map.md)。
